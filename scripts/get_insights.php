...
foreach ($rows as $row) {
    ...
    $idStmt = $pdo->prepare("SELECT id FROM categories WHERE name = :name AND parent_id IS NULL");
    $idStmt->execute([':name' => $category]);
    $catId = $idStmt->fetchColumn();
    ...
    if ($catId) {
        ...
        if ($budgeted !== null && -$actual > $budgeted) {
            ...
            error_log("Rule 1 SKIPPED: {$category} — actual: {$actual}, budgeted: " . ($budgeted ?? 'null'));
        }
    } else {
        error_log("Rule 1 SKIPPED: No category ID found for '{$category}'");
    }
}
