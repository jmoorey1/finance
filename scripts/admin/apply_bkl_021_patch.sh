...
mkdir -p "${ROOT}/scripts/admin"
mkdir -p "${ROOT}/migrations"
mkdir -p "${ROOT}/docs"

cat > "${ROOT}/scripts/admin/migration_lib.php" <<'PHP'
<?php
/**
...
cat > "${ROOT}/scripts/admin/migrate.php" <<'PHP'
<?php
...
cat > "${ROOT}/scripts/admin/new_migration.php" <<'PHP'
<?php
if (PHP_SAPI !== 'cli') {
    fwrite(STDERR, "This script must be run from the command line.\n");
    exit(1);
...
cat > "${ROOT}/migrations/20260415_000000_baseline_current_schema.sql" <<'SQL'
...
cat > "${ROOT}/docs/MIGRATIONS.md" <<'MD'
# Database Migrations
