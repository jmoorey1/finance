...
CREATE TABLE `predicted_transactions` (
  ...
  `active` tinyint(1) DEFAULT '1',
  `recurrence_unit` enum('week','month','year') NOT NULL,
  `recurrence_interval` int NOT NULL DEFAULT '1',
  `anchor_date` date NOT NULL,
  `schedule_pattern` enum('anchor_date','day_of_month','nth_weekday','month_end') NOT NULL DEFAULT 'anchor_date',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `weekday` tinyint DEFAULT NULL,
  `nth_weekday` tinyint DEFAULT NULL,
  `business_day_adjustment` enum('none','previous_business_day','next_business_day') NOT NULL DEFAULT 'none',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_predicted_transactions_recurrence` (`active`,`recurrence_unit`,`anchor_date`),
  ...
  CONSTRAINT `chk_predicted_transactions_day_of_month` CHECK (((`day_of_month` is null) or (`day_of_month` between 1 and 31))),
  CONSTRAINT `chk_predicted_transactions_nth_weekday` CHECK (((`nth_weekday` is null) or (`nth_weekday` between 1 and 5))),
  CONSTRAINT `chk_predicted_transactions_recurrence_interval` CHECK ((`recurrence_interval` >= 1)),
  CONSTRAINT `chk_predicted_transactions_weekday` CHECK (((`weekday` is null) or (`weekday` between 0 and 6)))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
